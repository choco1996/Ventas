...
<div class="row">
             ...
             <div class="form-group col-sm-2">
                 <asp:Button ID="btnbusqueda" Text="Busqueda" runat="server" CssClass="btn btn-primary" OnClick="Unnamed1_Click"/>
            </div>
</div>
         ...
         <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:VentasConnectionString %>" SelectCommand="SELECT [idclientes], [nombre], [apellido], [rtn], [direccion], [telefono], [correo] FROM [clientes] WHERE ([idclientes] LIKE '%' + @idclientes + '%')">
    <SelectParameters>
        <asp:ControlParameter ControlID="txtbusqueda" Name="idclientes" PropertyName="Text" Type="String" />
    </SelectParameters>
         </asp:SqlDataSource>
         <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:VentasConnectionString %>" SelectCommand="SELECT idclientes, nombre, apellido, rtn, direccion, telefono, correo FROM clientes WHERE (apellido LIKE '%' + @apellido + '%')">
    <SelectParameters>
        <asp:ControlParameter ControlID="txtbusqueda" Name="apellido" PropertyName="Text" Type="String" DefaultValue="" />
    </SelectParameters>
         </asp:SqlDataSource>
          <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:VentasConnectionString %>" SelectCommand="SELECT [idclientes], [nombre], [apellido], [rtn], [direccion], [telefono], [correo] FROM [clientes]">
         </asp:SqlDataSource>
